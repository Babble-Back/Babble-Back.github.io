...
add column if not exists guess_events jsonb not null default '[]'::jsonb;

alter table public.rounds
add column if not exists guess_mistake_count integer;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'rounds_guess_events_array'
      and conrelid = 'public.rounds'::regclass
  ) then
    alter table public.rounds
    add constraint rounds_guess_events_array
    check (jsonb_typeof(guess_events) = 'array');
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'rounds_guess_mistake_count_nonnegative'
      and conrelid = 'public.rounds'::regclass
  ) then
    alter table public.rounds
    add constraint rounds_guess_mistake_count_nonnegative
    check (guess_mistake_count is null or guess_mistake_count >= 0);
  end if;
end;
$$;
...
begin
  ...
  if normalized_mistake_count is not null then
    expected_score := case
      ...
      when normalized_mistake_count <= 2 then 8
      when normalized_mistake_count <= 4 then 5
      else 0
    end;

    if score_input is distinct from expected_score then
      raise exception 'Score does not match the guess mistake count.';
    end if;
  end if;
