  ...
  if normalized_mistake_count is not null then
    with target_letters as (
      select
        ...
        substr(round_row.correct_phrase, phrase_positions.position, 1) as expected
      from generate_series(1, char_length(round_row.correct_phrase)) as phrase_positions(position)
      where substr(round_row.correct_phrase, phrase_positions.position, 1) !~ '^\s$'
    ),
    event_values as (
      select
        case
      ...
      from jsonb_array_elements(normalized_guess_events) as event(value)
    ...
    expected_score := case
      when target_letter_count = 0 and normalized_mistake_count = 0 then 10
      when target_letter_count = 0 then 0
      when correct_letter_count = target_letter_count and normalized_mistake_count = 0 then 10
      when correct_letter_count::numeric / target_letter_count > (2.0 / 3.0)
        and normalized_mistake_count < 2 then 8
      when correct_letter_count::numeric / target_letter_count > (1.0 / 3.0)
        and normalized_mistake_count <= 4 then 5
      else 0
    end;
...
as $$
begin
  return public.complete_round_and_award_resources(
    round_id,
    guess_input,
    score_input,
    difficulty_input,
